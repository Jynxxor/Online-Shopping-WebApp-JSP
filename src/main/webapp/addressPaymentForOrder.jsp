...
try{
    ...
    ResultSet rs1 = stm.executeQuery("SELECT sum(total) FROM cart WHERE  email='"+email+"' AND address is NULL;");
    if(rs1.next()){
        total = rs1.getInt(1);
    }
%>
        ...
        <tbody>
        <%
      ResultSet rs = stm.executeQuery("SELECT * FROM product inner join cart on product.id = cart.product_id AND cart.email = '"+email+"' AND cart.address is NULL;");
      while(rs.next()){
      %>
          <tr>
          <% sno += 1; %>
           <td><%=sno%></td>
            ...
            <td><i class="fa fa-inr"></i> <td><%=rs.getString(4) %></td></td>
            <td> <td><%=rs.getString(8) %></td></td>
            <td><i class="fa fa-inr"></i> <td><%=rs.getString(10) %></td></td>
            </tr>
      <% }
      ResultSet rs2 = stm.executeQuery("SELECT * FROM users WHERE email='"+email+"';");
      while(rs2.next()){
      %>
        </tbody>
      </table>
...
<input class="input-style" type="text" name="transactionId" placeholder="Enter Transaction ID" id="transactionId" required> 
...
paymentMethod.addEventListener('change', () => {
    if (paymentMethod.value === 'Cash on delivery(COD)') {
    	transactionId.setAttribute('readonly', true);
